...
- `Mesh rectprism(float X, Y, Z = 16)` = Generates a rectangular prism of sides `[X, Y, Z]` centered at the origin. Each face has the same texture mapping.
- `Mesh rectprism(Vector3 v)` = Generates a rectangular prism of sides `v` centered at the origin. Each face has the same texture mapping.
- `Mesh rectprism(Vector3 size, Vector3 pos)` = Generates a rectangular prism of sides `v` centered at the `pos`. Each face has the same texture mapping.
