...
template <typename T>
static int getIndexOrCreate(std::vector<T>& v, std::map<T, int>& inv, T i) {
...
static Mesh subdivide(Mesh m, int k = 1) {
    if (k <= 0) return m;
...
static Mesh subdivideLoop(Mesh m, int k = 1) {
    if (k <= 0) return m;
...
static SharpQuadMesh subdivideCC(SharpQuadMesh m, int k = 1) {
    if (k <= 0) return m;
...
static QuadMesh subdivideCC(QuadMesh m, int k = 1) {
    if (k <= 0) return m;
    if (k > 1) m = subdivideCC(m, k - 1);
