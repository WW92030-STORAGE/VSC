...
template <typename T>
static T* fromvec(std::vector<T> s) {
	int N = s.size();
	T* res = new T[N];
...
template <typename T>
static int pog(std::vector<T>& arr, std::map<T, int>& inv, T object) {
...
static std::vector<float> operator+(std::vector<float> const &a, std::vector<float> const & b) {
    std::vector<float> res;
    for (int i = 0; i < a.size() && i < b.size(); i++) res.push_back(a[i] + b[i]);
    return res;
}

static std::vector<float> operator*(std::vector<float> const &a, float b) {
    std::vector<float> res;
...
static std::vector<float> ket(int n, int k) {
...
template <typename T>
static std::vector<T> vec_slice(std::vector<T> value, int start = 0, int end = -1) {
