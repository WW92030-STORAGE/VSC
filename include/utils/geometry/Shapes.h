...
static Mesh GridSquare(float X = 16, int N = 1) {
	X = fabs(X);
	N = BASE::max(N, 1);
...
static Mesh cylinder(float R = 1, float H = 1, int X = 16) {
	if (X < 3) X = 3;
...
static int VFE_sphere(std::vector<Vector3>& vertices, std::map<std::pair<int, int>, int>& lookup, int f, int s) {
...
static std::vector<std::vector<int>> subdivide_sphere(std::vector<Vector3>& verts, std::vector<std::vector<int>>& tris) {
...
static Mesh icosphere(float R = 1, int S = 1) {
	// Failsafe
	S = BASE::min(S, 5);
