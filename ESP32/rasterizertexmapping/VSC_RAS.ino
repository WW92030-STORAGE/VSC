...
void loop() {
  ...
  ImageTexture mat = ImageTexture(TEX::rgbcubesmall);
  BaseMaterial WHITE(0xFFFFFFFF, 64);

  s.clearBuffer();
  if (flashbyte == 1) s.fillMesh(proto, &mat, true, true, false);
  else if (flashbyte == 2) s.fillMesh(proto, &WHITE, true, false);
  RenderBuffer(s);
  disp();
