...
This program, given a file, generates a C++ function to return the contents of the file as a vector of strings, each entry one line.
...
def output():
	...
	string = string + "\tstd::vector<std::string> " + sn + "(" + str(LEN) + ");\n"
	...
	for line in FILE:
		...
		if ignore_useless and (len(line.strip()) <= 0 or line[0] == '#'):
			continue
		string = string + "\t" + sn + "[" + str(INDEX) + "] = (\"" + line.strip() + "\\n\");\n"
