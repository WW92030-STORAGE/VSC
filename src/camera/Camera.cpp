...
#include "../../include/camera/Camera.h"
	...
	Vector3 Camera::right() {
		return (transform.basis.xAxis);
	}

	Vector3 Camera::up() {
		return (transform.basis.yAxis);
	}

	Vector3 Camera::look() {
		return (transform.basis.zAxis);
	}
	...
	Camera::Camera(float a, float N, float F) {
		alpha = a;
		n = N;
		f = F;
	...
	float Camera::getNearPlaneBorders() {
	...
	Vector2 Camera::rasterize(Vector3 p) {
	...
	// Get the z value
	float Camera::perserpz(Vector3 p, Vector3 q, float t) {
	...
	float Camera::perserp(Vector3 p, Vector3 q, float t) {
	...
	float Camera::verp(Vector3 p, Vector3 q, float b1, float b2, float t) {
	...
	Vector4 Camera::glFrustum(Vector4 p) {
	...
	Vector4 Camera::glFrustumNoBack(Vector4 p) {
	...
	Vector4 Camera::glOrtho(Vector4 p) {
		Vector4 res = Ortho() * p;
		return res;
	}