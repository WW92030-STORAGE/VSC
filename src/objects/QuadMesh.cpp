...
#include "../../include/objects/QuadMesh.h"

	QuadMesh::QuadMesh() { // Default cube
	...
	QuadMesh::QuadMesh(int sz) {
		size = sz;
		quadindices = std::vector<std::vector<int>>(size, std::vector<int>(4, 0));
	}

	QuadMesh::QuadMesh(Vector3* v, int** t, int nv, int sz) {
		nverts = nv;
		size = sz;
	...
	QuadMesh::QuadMesh(Vector3* v, int** t, Vector2* tex, int** texco, int nv, int sz, int nuuv) {
		nverts = nv;
		size = sz;
		nuv = nuuv;
	...
	QuadMesh::QuadMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t) {
		size = t.size();
		nverts = v.size();
	...
	QuadMesh::QuadMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t,std::vector<Vector2> tex, std::vector<std::vector<int>>& texco) {
		size = t.size();
		nverts = v.size();
		nuv = tex.size();
	...
	QuadMesh::QuadMesh(const QuadMesh& other) : Object(other) {
		size = other.size;
		nverts = other.nverts;
		nuv = other.nuv;
		verts = std::vector<Vector3>(nverts);
	...
	Vector3 QuadMesh::centroid(int i) {
		Vector3 res(0, 0, 0);
	...
	QuadMesh QuadMesh::fromOBJ(std::string filename) {
	...
	bool SharpQuadMesh::isSharp(Vector3 v) {
		return shverts.find(v) != shverts.end();
	}

	bool SharpQuadMesh::isSharp(std::pair<Vector3, Vector3> v) {
		std::pair<Vector3, Vector3> u = {v.second, v.first};
		return shedges.find(v) != shedges.end() || shedges.find(u) != shedges.end();
	}

	
	SharpQuadMesh::SharpQuadMesh(int sz) : QuadMesh(sz) {
	}

	SharpQuadMesh::SharpQuadMesh(Vector3* v, int** t, int nv, int sz) : QuadMesh(v, t, nv, sz) {
	}

	SharpQuadMesh::SharpQuadMesh(Vector3* v, int** t, Vector2* tex, int** texco, int nv, int sz, int nuuv) : QuadMesh(v, t, tex, texco, nv, sz, nuuv) {
	}

	SharpQuadMesh::SharpQuadMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t) : QuadMesh(v, t) {
		
	}

	SharpQuadMesh::SharpQuadMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t,std::vector<Vector2> tex, std::vector<std::vector<int>>& texco) : QuadMesh(v, t, tex, texco) {
	}

	SharpQuadMesh::SharpQuadMesh(const SharpQuadMesh& other) : QuadMesh(other) {
		shedges = other.shedges;
		shverts = other.shverts;
	}

	SharpQuadMesh::SharpQuadMesh(const QuadMesh& other) : QuadMesh(other) {
		
	}

	void SharpQuadMesh::Trans(Transform t) {
		transform = t * transform;
		Matrix4 trans = t.matrix();
		for (int i = 0; i < nverts; i++) verts[i] = vec3(trans * fromPoint(verts[i]));
		std::set<Vector3> shv;
	...
	void SharpQuadMesh::ForceTrans(Transform t) {
		Matrix4 trans = t.matrix();
		for (int i = 0; i < nverts; i++) verts[i] = vec3(trans * fromPoint(verts[i]));
