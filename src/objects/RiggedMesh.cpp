...
#include "../../include/utils/geometry/Shapes.h"

#include "../../include/objects/Mesh.h"

#include "../../include/objects/RiggedMesh.h"

    RiggedMesh::RiggedMesh(const RiggedMesh& other) : Mesh(other) {
		size = other.size;
		nverts = other.nverts;
		nuv = other.nuv;
    ...
    void RiggedMesh::Trans(Transform t) {
    ...
    void RiggedMesh::ForceTrans(Transform t) {
		Matrix4 trans = t.matrix();
		for (int i = 0; i < nverts; i++) {
            verts[i] = vec3(trans * fromPoint(verts[i]));
            original_verts[i] = vec3(trans * fromPoint(original_verts[i]));
    ...
    RiggedMesh::RiggedMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t) : Mesh(v, t){
        init();
    }

    RiggedMesh::RiggedMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t,std::vector<Vector2> tex, std::vector<std::vector<int>>& texco) : Mesh(v, t, tex, texco){
        size = t.size();
		nverts = v.size();
		nuv = tex.size();
    ...
    RiggedMesh::RiggedMesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t,std::vector<Vector2> tex, std::vector<std::vector<int>>& texco, 
    ...
    bool RiggedMesh::is_root(int i) {
        return parent[i] < 0;
    }


    void RiggedMesh::computeAbsoluteTransforms() {
        std::queue<int> q;
    ...
    void RiggedMesh::computeVertexLocations() {
        for (int i = 0; i < nverts; i++) {
            float sum = 0.0;
    ...
    std::vector<Mesh> RiggedMesh::visBones(float R) {
        std::vector<Mesh> res;
    ...
    void RiggedMesh::deform(std::string name, Transform t) {
