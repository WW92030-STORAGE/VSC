...
#include "../../include/objects/Mesh.h"
	...
	*/
	...
	Mesh::Mesh(Vector3* v, int** t, int nv, int sz) {
		nverts = nv;
		size = sz;
	...
	Mesh::Mesh(Vector3* v, int** t, Vector2* tex, int** texco, int nv, int sz, int nuuv) {
		nverts = nv;
		size = sz;
		nuv = nuuv;
	...
	Mesh::Mesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t) {
		size = t.size();
		nverts = v.size();
	...
	Mesh::Mesh(std::vector<Vector3>& v, std::vector<std::vector<int>>& t,std::vector<Vector2> tex, std::vector<std::vector<int>>& texco) {
		size = t.size();
		nverts = v.size();
		nuv = tex.size();
	...
	Mesh::Mesh(std::vector<Triangle3> tt) {
		int sz = tt.size();
		size = sz;
		triindices = std::vector<std::vector<int>>(size, std::vector<int>(3));
		texcoords = std::vector<std::vector<int>>(size, std::vector<int>(3));
	...
	void Mesh::ForceTrans(Transform t) {
		Matrix4 trans = t.matrix();
		for (int i = 0; i < nverts; i++) verts[i] = vec3(trans * fromPoint(verts[i]));
	...
	int Mesh::comp (const void * a, const void * b) {
	...
	Vector3 Mesh::getVertexNormal(int i) {
		return vn[i];
	}

	Vector3 Mesh::getVertexNormal(int ind, int i) {
		return getVertexNormal(triindices[ind][i]);
	}

	// Get normal for point in triangle, interpolated
	Vector3 Mesh::getInterpolatedNormal(Vector3 pos, int ind) {
	...
	Vector2 Mesh::getUV(Vector3 pos, int ind) {
		Triangle3 tri = makeTriangle(ind);
	...
	Triangle3 Mesh::makeTriangle(int i) {
