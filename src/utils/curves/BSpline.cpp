...
#include "../../../include/utils/curves/BSpline.h"

// Uniform cubic B-spline curve
    
    CubicBSpline::CubicBSpline() : BaseCurve() {
    }

    CubicBSpline::CubicBSpline(std::vector<Vector3> v) : BaseCurve(v) {
    }

    // Knots "off the track" are duplicated and clamped.
    Vector3 CubicBSpline::knot(int i) {
    ...
    // order = degree + 1
    Vector3 CubicBSpline::query(float t) {
    ...
    std::string CubicBSpline::to_string() {
    ...
    CubicBSplineSurface::CubicBSplineSurface() : BaseSurface() {

    }

    CubicBSplineSurface::CubicBSplineSurface(std::vector<std::vector<Vector3>> v) : BaseSurface(v) {
        
    }

    std::vector<Vector3> CubicBSplineSurface::getUCurve(float u) {
    ...
    Vector3 CubicBSplineSurface::query(float u, float v) {
        u = BASE::clamp(u, 0, 1);
        v = BASE::clamp(v, 0, 1);
