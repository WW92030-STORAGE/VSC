...
// Matrix2
	...
	Matrix2::Matrix2(Vector2 r, Vector2 s) {
		xAxis = Vector2(r);
		yAxis = Vector2(s);
	}
	...
	Matrix2::Matrix2(float r, float s, float t, float u) {
		xAxis = Vector2(r, t);
		yAxis = Vector2(s, u);
	}
	...
	Matrix2 Matrix2::sub(const Matrix2& other) {
		return Matrix2(xAxis - other.xAxis, yAxis - other.yAxis);
	}
	Matrix2 Matrix2::mul(float other) {
		return Matrix2(xAxis * other, yAxis * other);
	}
	Matrix2 Matrix2::div(float other) {
...
// 3x3 matrices
	...
	Matrix3::Matrix3(Vector3 r, Vector3 s, Vector3 t) {
		xAxis = (r);
		yAxis = (s);
		zAxis = (t);
	}
	...
	Matrix3 Matrix3::eye_scaled(float f) {
		return Matrix3(f, 0, 0, 0, f, 0, 0, 0, f);
	}

	Matrix3 Matrix3::diagonal(float a, float b, float c) {
		return Matrix3(a, 0, 0, 0, b, 0, 0, 0, c);
	}

	// get a (row, column)
	float Matrix3::get(int a, int b) {
		if (b == 0) {
	...
	void Matrix3::set(int a, int b, float v) {
		if (b == 0) {
			if (a == 0) xAxis.x = v;
	...
	Vector3 Matrix3::getCol(int a) {
		if (a == 0) return (xAxis);
		if (a == 1) return (yAxis);
		return (zAxis);
	}

	void Matrix3::setCol(int a, Vector3 v) {
		if (a == 0) xAxis = (v);
		else if (a == 1) yAxis = (v);
		else zAxis = (v);
	}

	Vector3 Matrix3::getRow(int a) {
	...
	Matrix3 Matrix3::sub(const Matrix3& other) {
		return Matrix3(xAxis - other.xAxis, yAxis - other.yAxis, zAxis - other.zAxis);
	}
	Matrix3 Matrix3::mul(float other) {
		return Matrix3(xAxis * other, yAxis * other, zAxis * other);
	}
	Matrix3 Matrix3::div(float other) {
	...
	Vector3 Matrix3::vecmul(const Vector3& other) {
		return Vector3(getRow(0).dot(other), getRow(1).dot(other), getRow(2).dot(other));
	}

	Matrix3 Matrix3::matmul(const Matrix3& other) {
...
// 4x4 matrices
	...
	Matrix4::Matrix4(Vector4 r, Vector4 s, Vector4 t, Vector4 u) {
		xAxis = (r);
		yAxis = (s);
		zAxis = (t);
		wAxis = (u);
	...
	void Matrix4::set(int a, int b, float v) {
		if (b == 0) {
			if (a == 0) xAxis.x = v;
			else if (a == 1) xAxis.y = v;
	...
	void Matrix4::setRow(int a, Vector4 v) {
		set(a, 0, v.x);
		set(a, 1, v.y);
		set(a, 2, v.z);
		set(a, 3, v.w);
	...
	Matrix4 Matrix4::mul(float other) {
		return Matrix4(xAxis * other, yAxis * other, zAxis * other, wAxis * other);
	}
	Matrix4 Matrix4::div(float other) {
		if (other == 0) return eye();
		float oo = 1.0 / other;
	...
	Vector4 Matrix4::vecmul(const Vector4& other) {
		Vector4 res;
		const int N = 4;
	...
	Matrix4 Matrix4::matmul(const Matrix4& other) {
