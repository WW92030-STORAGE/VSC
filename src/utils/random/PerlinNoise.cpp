...
#include "../../../include/utils/random/PerlinNoise.h"
    ...
    PerlinNoise2::PerlinNoise2() : NoiseOctave2() {
        init(0);
    }

    PerlinNoise2::PerlinNoise2(uint64_t sed) : NoiseOctave2(seed) {
        init(sed);
    }

    void PerlinNoise2::init(uint64_t sed) {
        seed = sed;
        rs->init(sed);
        xm = rs->rand64();
        xb = rs->rand64();
    ...
    Vector2 PerlinNoise2::randvec(int64_t x, int64_t y) {
        rs->init(x * xm + xb);
    ...
    double PerlinNoise2::smooth(double x) {
        return 0.5 * (1 - cosf(M_PI * x));
    }

    double PerlinNoise2::rand(double x, double y) {
